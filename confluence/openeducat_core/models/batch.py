...
class OpBatch(models.Model):
    ...
    level_id = fields.Many2one('op.level', 'Grade', required=True)
    ...
    def name_search(self, name, args=None, operator='ilike', limit=100):
        if self.env.context.get('get_parent_batch', False):
            lst = []
            lst.append(self.env.context.get('level_id'))
            levels = self.env['op.level'].browse(lst)
            batches = self.env['op.batch'].search([('level_id', 'in', lst)])
