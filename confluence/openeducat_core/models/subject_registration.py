...
class OpSubjectRegistration(models.Model):
    ...
    _sql_constraints = [
        ('unique_student_id_subj_registration',
         'unique(student_id,level_id,batch_id,compulsory_subject_ids)',
         'Subject Registration must be unique per Batch!'),
    ]
    ...
    def action_approve(self):
        for record in self:
            ...
            for sub in record.elective_subject_ids:
                subject_ids.append(sub.id)
            level_id = self.env['op.student.level'].search([
            ...
            if level_id:
                ...
                raise ValidationError(
                    _("Level not found on student's admission!"))
    ...
    def get_subjects(self):
        for record in self:
            subject_ids = []
            if record.level_id and record.level_id.subject_ids:
                for subject in record.level_id.subject_ids:
