...
class OpSession(models.Model):
    ...
    def _compute_batch_users(self):
        ...
        for session in self:
            student_ids = student_obj.search(
                [('level_detail_ids.batch_id', '=', session.batch_id.id)])
    ...
    def create(self, values):
        ...
        if res.batch_id and res.level_id:
            level_val = self.env['op.student.level'].search([
    ...
    def onchange_level(self):
        self.batch_id = False
        if self.level_id:
            subject_ids = self.env['op.level'].search([
                ('id', '=', self.level_id.id)]).subject_ids
